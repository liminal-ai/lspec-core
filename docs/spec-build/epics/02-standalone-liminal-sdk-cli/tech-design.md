...
`INTERNAL_ERROR` is reserved for genuinely unexpected failures (uncaught throws inside the package's own code that don't have a typed error class). Consumers branch on `error.code`; `instanceof` is the secondary classification mechanism for in-process callers. String matching against `error.message` is forbidden in non-test code. **Scope clarification (Story 3 acceptance ruling, 2026-04-28):** TC-4.2b applies to classification of *internal typed error objects* — `Error.message`, blocker arrays, and any string field on objects produced by package code. It does NOT apply to *external-protocol parsing at provider-adapter boundaries*, where the only available signal is unstructured provider stdout/stderr text and the parsing produces a typed result object (e.g. `parseAuthCheckOutcome(stderr): AuthCheckOutcome`). External-protocol parsing is by definition text-driven; the rule it must satisfy is "yield a typed result," not "use no text inspection." `src/core/provider-checks.ts` `parseAuthCheckOutcome` is the canonical example.
